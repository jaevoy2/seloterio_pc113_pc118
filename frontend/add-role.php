                        ...
                        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item" aria-current="page">
                                    <a class="text-decoration-none text-dark" style="font-size: 13px">Add Role</a>
                                </li>
                            </ol>
                        </nav>
                                            ...
                                            <div class="pt-2">
                                                <form action="" method="POST" enctype="multipart/form-data">
                                                    <div class="">
                                                        ...
                                                        <div class="mt-3">
                                                            <h6>Permissions</h6>
                                                            <div class="d-flex flex-wrap gap-5 mt-2 p-3" style="background-color:rgb(242, 242, 242); border-radius: 5px; font-size: 13px" id="permissionContainer"></div>
                                                        </div>
                                                    </div>
...
<!-- display role permission -->
    <script>
        $(document).ready(function() {
            fetch('https://backend-folder.test/api/admin/roles-permissions', {
                method: 'GET',
                headers: {
            ...
            .then(res => res.json())
            .then(data => {
                let permissionCon = document.getElementById('permissionContainer');
...
<!-- add role -->
     <script>
        $(document).on('click', '#saveRole', function(event) {
            ...
            let permissions = [];
            document.querySelectorAll('.checked-permission:checked').forEach((checkbox) => {
                permissions.push(checkbox.value);
            })
        
            $.ajax({
                url: 'https://backend-folder.test/api/admin/save-role',
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Authorization': 'Bearer ' + localStorage.getItem('token')
                ...
                success: function(response) {
                    if(!response.message) {
                        ...
                        Swal.fire({
                            ...
                            location.href = 'role_permission.php';
                        });
                    }
                },
                complete: function() {
